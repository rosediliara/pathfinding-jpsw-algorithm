...
warthog::offline_jump_point_locator::preproc()
{
    ...
    std::cerr << "computing jump point database\n";
...
//				std::cout << dir << ": ";
				...
				jpl.jump(dir, mapid,
						warthog::INF, jumpnode_id, num_steps);
//				std::cout << (jumpnode_id == warthog::INF ? 0 : num_steps) << " ";
				...
				if(num_steps >= DEADEND)
				{
					std::cerr 
                        << "label overflow; maximum jump distance "
                        << "exceeded. aborting\n";
					exit(1);
				}
			}
...
warthog::offline_jump_point_locator::load(const char* filename)
{
	...
	std::cerr << "looking for database "<<fname << "... ";
	if(f == NULL) 
	{
...
warthog::offline_jump_point_locator::save(const char* filename)
{
	...
	strcat(fname, ".jps+");
	std::cerr 
        << "saving to file "<<fname<<"; #labels="<<dbsize_
        <<" label size: "<<sizeof(db_[0]) << " bytes"<<std::endl;
	...
	fclose(f);
	std::cerr << "jump-point db saved to disk. file="<<fname<<std::endl;
}
	  	...
	  	uint32_t goal_id, uint32_t& jumpnode_id, warthog::cost_t& jumpcost)
{
	...
	jumpcost = num_steps;
	if(label & DEADEND) { jumpnode_id = warthog::INF; }
}
